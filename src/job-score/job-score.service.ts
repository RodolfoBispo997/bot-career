...
import { JobTrack, RoleType, SearchProfile, Seniority } from '../search-profile/types';
import {
...
import { JobPriority, JobScoreInput, JobScoreResult, ScoreComponent } from './types';

@Injectable()
export class JobScoreService {
  evaluate({ job, classification, decision, profile }: JobScoreInput): JobScoreResult {
    ...
    const breakdown = {
      ...
      role: this.component(ROLE_WEIGHTS[classification.roleType], this.roleMax(classification.track)),
  ...
  private stackScore(classification: JobScoreInput['classification']): ScoreComponent {
    ...
    return this.component(score, classification.track === JobTrack.NODE_INTERNSHIP ? 45 : 40);
  }

  private seniorityScore(classification: JobScoreInput['classification']): ScoreComponent {
    const max = classification.track === JobTrack.PHP ? 25 : classification.track === JobTrack.NODE_INTERNSHIP ? 15 : 20;
    return this.component(SENIORITY_WEIGHTS[classification.track][classification.seniority] ?? 0, max);
  }
  ...
  private complementaryScore(
    ...
    const max = classification.track === JobTrack.NODE ? NODE_COMPLEMENTARY_MAX : classification.track === JobTrack.PHP ? PHP_COMPLEMENTARY_MAX : INTERNSHIP_COMPLEMENTARY_MAX;
    if (max === 0) return this.component(0, max);

    const configured = classification.track === JobTrack.NODE
      ? [...(profile.node.technologies.strong ?? []), ...(profile.node.technologies.complementary ?? [])]
      : [...(profile.php.technologies.strong ?? [])];
    const detected = classification.detectedSkills.filter((skill) => configured.includes(skill));
    return this.component(Math.min(max, detected.length), max);
  }

  private employmentScore(type: EmploymentType, track: JobTrack): number {
    if (track === JobTrack.NODE_INTERNSHIP) return type === EmploymentType.INTERNSHIP ? 10 : type === EmploymentType.UNKNOWN ? 5 : 0;
  ...
  private legacyPenalty(classification: JobScoreInput['classification']): number {
    if (classification.track !== JobTrack.PHP) return 0;
    const count = classification.detectedSkills.filter((skill) => LEGACY_SKILLS.includes(skill)).length;
  ...
  private positiveMessages(
    ...
    const positives: string[] = [`${classification.primaryStack} faz parte da stack principal`];
    positives.push(...classification.positiveSkills.slice(0, 5).map((skill) => `${skill} detectado`));
    if (classification.seniority !== Seniority.UNSPECIFIED) {
      positives.push(`Senioridade ${classification.seniority} considerada no perfil`);
    }
    if (job.workMode === WorkMode.REMOTE) positives.push('Vaga remota');
    if (breakdown.complementarySkills.score > 0) positives.push('Skills complementares relevantes detectadas');
    return positives;
  }
  ...
  private penaltyMessages(
    ...
    if (classification.track === JobTrack.NODE && classification.seniority === Seniority.MID) {
      penalties.push('Senioridade Pleno está acima do foco principal Node.js');
    }
    if (this.legacyPenalty(classification) > 0) penalties.push('Stack PHP contém tecnologia legacy');
    if (classification.warnings.some((warning) => warning.includes('força semelhante'))) {
